...
# Install pudu-agent
if [ -f "$AGENT_BIN" ]; then
    echo "==> Installing pudu-agent..."
    sudo cp "$AGENT_BIN" "$MOUNTPOINT/usr/local/bin/pudu-agent"
    sudo chmod +x "$MOUNTPOINT/usr/local/bin/pudu-agent"

    # Install systemd service unit
    cat <<'UNIT' | sudo tee "$MOUNTPOINT/etc/systemd/system/pudu-agent.service" > /dev/null
[Unit]
Description=Pudu Fault Injection Agent
After=network.target

[Service]
ExecStart=/usr/local/bin/pudu-agent
Restart=always
RestartSec=2
...
UNIT
    ...
    sudo chroot "$MOUNTPOINT" systemctl enable pudu-agent > /dev/null 2>&1 || true
    echo "  ✓ pudu-agent installed and enabled"
else
    echo "  (skipping pudu-agent: $AGENT_BIN not found)"
fi
